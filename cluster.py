...
class KMeans():
    def __init__(self, n_clusters, max_iter=100, verbose=0) -> None: #pridat pak random state?
    ...
    def objective_function(self,K,partition): #je tady i v heuristics, mozna presunout do nejake nadtridy?
        obj = 0 
        for c in range(0, self.n_clusters):
...
class Heuristics(): #mozna to pojmenovat random descent?
    def __init__(self, n_clusters, max_iter=10000, verbose=0) -> None: 
        self.n_clusters = n_clusters
    ...
    def fit(self, K, mutation, params = "default"):
        ...
        if params == "default":
            mut_curr = self.mutation_dic[mutation](0, self.n_clusters-1, verbose=self.verbose) 
        else:
